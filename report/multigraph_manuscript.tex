...
walks on each graph \cite{kondor}, calculating the so-called
$n$-tangle density \cite{fefferman}, or calculting the edit distance
with an approximation algorithm \cite{riesen,zeng}.  \\

The strategy used in the following methods, detailed in
\cite{rajendran_analysis_2013} and \cite{gu} enumerates the number of
times a certain set of motifs (or subgraphs) appears in each network
in the dataset. This maps each network to a vector in $\mathbb{R}^n$,
and the Euclidean distance is subsequently used to calculate the
similarity of two graphs. Due to computational restrictions, we chose
to count the number of three- and four-vertex subgraphs that were
contained in each network. As there are eight such motifs, shown in
Fig. (\ref{fig:motifs}), this process $\gamma$ maps each graph to an
eight-dimensional vector: $\gamma : G \rightarrow
\mathbb{R}^8$. Additionally, we applied this operateration to a
simplified version of each graph, wherein any multiple edges were
