...
namespace eigen_solver {
  ...
  int pca(const Eigen::MatrixXd& X, Eigen::MatrixXd& V, Eigen::VectorXd& S, const int k) {
    const int nvars = X.cols();
    const int nsamples = X.rows();
    Eigen::MatrixXd X_centered = X;
    // zero the averages
    for(int i = 0; i < nvars; i++) {
      X_centered.col(i) -= Eigen::VectorXd::Ones(nsamples)*(X_centered.col(i).sum()/nsamples);
    }
    
    int arnoldi_success = arnoldi_method_imprestart_hermitian(X.transpose()*X, Eigen::VectorXd::Random(nvars), V, S, k, 2*k, nvars, 2*nvars);
    return arnoldi_success;
  }
