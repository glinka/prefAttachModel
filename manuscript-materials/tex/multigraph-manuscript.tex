\documentclass[epjST, final]{svjour} \usepackage{graphicx, subcaption,
  setspace, indentfirst, amsfonts, amsthm, empheq}
% \usepackage[top=1in, bottom=1in, left=1in, right=1in]{geometry}
% \usepackage[center]{titlesec}
\graphicspath{ {../figs/} }
% \pagestyle{plain} \renewcommand\thesection{\Roman{section}}
...
%
  ...
  % \ignore{an Erd\H{o}s-R\'{e}nyi random graph on $n$ vertices and
  % $m$ edges. Here, however, $m$ may be larger than the total number
  % of edges in a completely-connected $n$-vertex graph so we cannot
  % assign a probability to each edges existence, instead placing $m$
  % edges at random in the graph.}
...
%
  ...
  with degree,
  %%% YGK immediately? after a short time ? at steady state ?  if at
  %%% steady state say it ?  ATH This refers to the dynamics at all
  %%% times. At every step, we need some probability of choosing a
  %%% given vertex v_i: if k -> 0, this probability is proportional to
  %%% deg(v_i), and if k -> infinity this probability is uniform over
  %%% all vertices (and degree has no effect. Valid at every step.
  while
  $\lim\limits_{\kappa \rightarrow \infty} P(v_k = v_l) = \frac{1}{n}
  \; \forall \; l$, and individual degrees have no effect.
  %%% YGK you mean individual initial degrees ?
%%%
  \par
...
%
  ...
  \begin{figure}[th!]
    ...
    \caption{Multigraph evolution dynamics for an $n=100$ vertex graph
      observed through the degree sequence (after sorting the nodes by
      degree). Two distinct transients are shown, initialized with (a)
      an Erd\H{o}s-R\'{e}nyi random graph with $m = 5,050$ edges and (b)
      a graph in which half the vertices are isolated, and half
      uniformly share $m = 5,050$ edges. Both approach the same ultimate
      stationary sequence. \label{fig:dse}}
  \end{figure}
  ...
  \end{figure}
  %%% 
  %%% YGK maybe you can draw a short horizontal broken line at the "end"
  %%% of the graph, where "presumably" the steady state value is
  %%% extending from the boundary of the plot "inward" in the plot maybe
  %%% a thick black broken line ?
  ...
  %%% YGK is this you, Alexander, is it Balazs, did you read it
  %%% somewhere, is there a reference ?  ATH This is me, but Balazs
  %%% confirmed this in his comments when he said that Havel Hakimi was
  %%% even overly-complex for this case as there are even simpler
  %%% methods one could use. I think it is easy to prove.
  ...
  %%% YGK do you say what your step counts are in your actual
  %%% computation ?  ATH added specifics into the paragraph below
  ...
  %%% YGK can you consistently make F and d bold, so that they are
  %%% vectors ?
  %%% 
  %%% ATH done
  ...
  %%% YGK why even intervals ?  at even intervals ATH I don't remember a
  %%% particular reason
  ...
  % 
  % The particular eigenvector each trajectory travels along is also
  % significant: as $G_e(t)$ was mapped to $\Phi_1$ and $G_l(t)$ to
  % $\Phi_2$, we can infer that the dynamics of the
  % Erd\H{o}s-R\'{e}nyi-initialized system occur more slowly than those
  % of the lopsided-initialized system. Indeed, referring back to
  % Fig. (\ref{fig:dse}), we see that the lopsided network descends more
  % rapidly to the stationary state.
  ...
  Newton-GMRES.
  % 
  Additionally, an underlying two-dimensional description was uncovered via PCA and DMAPS. \\

  An open area of investigation is the interpretation of the output from
  ...
  %% This poses several problems to the investigator. First, the
  %% interactions themselves may be complicated functions that are
  %% expensive to compute and dependent on several network
  %% parameters. This, coupled with the large network sizes inherent to
  %% many interesting problems makes numerical investigation
  %% computationally prohibitive.

  %% For instance, the simple model proposed in \ref{votingmodel}
  %% describing the adoption of opinions in a population is defined such
  %% that at each simulation step, a random individual is chosen and
  %% acted upon.  This precludes the use of traditional numerical
  %% techniques such as optimization and fixed-point location that could
  %% offer valuable insight into the network's behavior.  We expand
